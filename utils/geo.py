...
DATA_PATH = Path("data") / "mcneill_database.xlsx"
...
def load_provider_addresses():
    df = pd.read_excel(DATA_PATH, sheet_name="Provider")
    df["Full Address"] = (
...
def add_coordinates(df):
    provider_df = load_provider_addresses()

    df["Latitude"] = None
    df["Longitude"] = None

    for prop in df["Property Name"].unique():
        # Find the provider row for this property
